...
direction_matrix.to_csv(f,header=False)
f.close()
...
def fp_directions(direction_matrix):
	...
	for i in range(direction_matrix.shape[ 0 ]):
		if i % 2 == 0:
			plot_directions['long'].append(
				direction_matrix['start_lng'][start])
			plot_directions['lat'].append(
				direction_matrix['start_lat'][start])
			start = start + 1
		else:
			plot_directions['long'].append(
				direction_matrix['end_lng'][end])
			plot_directions['lat'].append(
				direction_matrix['end_lat'][end])
			end = end + 1
